...
async function getAllFriends() {
  const body = types.SyncAllRequest.encode(
    types.SyncAllRequest.create({ open_ids: [] }),
  ).finish();
...
async function putInsects(friendGid, landIds) {
  const body = types.PutInsectsRequest.encode(
    types.PutInsectsRequest.create({
      land_ids: landIds,
      host_gid: toLong(friendGid),
    }),
  ).finish();
...
function analyzeFriendLands(lands, myGid, friendName = "") {
  ...
  for (const land of lands) {
    ...
    // 是否显示此好友的调试信息
    const showDebug =
      DEBUG_FRIEND_LANDS === true || DEBUG_FRIEND_LANDS === friendName;
    ...
    if (showDebug) {
      ...
      console.log(
        `  [${friendName}] 土地#${id}: phase=${phaseVal} stealable=${plant.stealable} dry=${toNum(plant.dry_num)} weed=${weedOwners.length} bug=${insectOwners.length}`,
      );
    }
    ...
    if (phaseVal === PlantPhase.MATURE) {
      if (plant.stealable) {
        ...
        console.log(
          `  [${friendName}] 土地#${id}: 成熟但stealable=false (可能已被偷过)`,
        );
      }
      continue;
    }
...
async function visitFriend(friend, totalActions, myGid) {
  ...
  if (showDebug) {
    console.log(
      `  [${name}] 分析结果: 可偷=${status.stealable.length} 浇水=${status.needWater.length} 除草=${status.needWeed.length} 除虫=${status.needBug.length}`,
    );
    console.log(`========== 调试结束 ==========\n`);
  }
  ...
  if (status.needBug.length > 0) {
    ...
    if (shouldHelp) {
      ...
      for (const landId of status.needBug) {
        try {
          await helpInsecticide(gid, [landId]);
          ok++;
        } catch (e) {
          /* ignore */
        }
...
async function checkFriends() {
  ...
  try {
    ...
    for (const f of friends) {
      ...
      // 调试：显示指定好友的预览信息
      const showDebug =
        DEBUG_FRIEND_LANDS === true || DEBUG_FRIEND_LANDS === name;
      if (showDebug) {
        console.log(
          `[调试] 好友列表预览 [${name}]: steal=${stealNum} dry=${dryNum} weed=${weedNum} insect=${insectNum}`,
        );
      }
      ...
      if (showDebug && visitedGids.has(gid)) {
        console.log(
          `[调试] 好友 [${name}] 加入列表 (位置: ${priorityFriends.length})`,
        );
      }
    }
    ...
    if (DEBUG_FRIEND_LANDS && typeof DEBUG_FRIEND_LANDS === "string") {
      const idx = friendsToVisit.findIndex(
        (f) => f.name === DEBUG_FRIEND_LANDS,
      );
      if (idx >= 0) {
        const inPriority = idx < priorityFriends.length;
        console.log(
          `[调试] 好友 [${DEBUG_FRIEND_LANDS}] 位置: ${idx + 1}/${friendsToVisit.length} (${inPriority ? "优先列表" : "其他列表"})`,
        );
      } else {
    ...
    for (let i = 0; i < friendsToVisit.length; i++) {
      ...
      const showDebug =
        DEBUG_FRIEND_LANDS === true || DEBUG_FRIEND_LANDS === friend.name;
      if (showDebug) {
        console.log(
          `[调试] 准备访问 [${friend.name}] (${i + 1}/${friendsToVisit.length})`,
        );
      }
      try {
      ...
      if (!canOperate(10004) && !canOperate(10003)) {
        // 10004=放虫, 10003=放草
        // 继续巡查，但不再放虫放草
      }
    }
