...
async function sellItems(items) {
  ...
  const body = types.SellRequest.encode(
    types.SellRequest.create({ items: payload }),
  ).finish();
  const { body: replyBody } = await request(
    "gamepb.itempb.ItemService",
    "Sell",
...
function getBagItems(bagReply) {
  if (
    ...
    bagReply.item_bag.items.length
  )
...
async function debugSellFruits() {
  try {
    ...
    for (const item of items) {
      const id = toNum(item.id);
      const count = toNum(item.count);
      if (isFruitIdBySeedData(id) && count > 0) toSell.push(item);
    }
...
function startSellLoop(interval = 60000) {
  if (sellTimer || sellStartTimer) return;
